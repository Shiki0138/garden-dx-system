...
export const calculateProcessDuration = (item, projectScale, season) => {
  ...
  const finalDuration = Math.max(
    1,
    Math.round(baseDuration * amountFactor * scaleFactor * seasonFactor)
  );
...
export const optimizeSchedule = (processes, projectStartDate) => {
  ...
  const findReadyProcesses = () => {
    return processes.filter(process => {
      ...
      return process.dependencies.every(depId => completedProcesses.has(depId));
    });
  };
  ...
  const findParallelProcesses = readyProcesses => {
    ...
    LANDSCAPING_PROCESS_PATTERNS.parallelProcesses.forEach(parallelGroup => {
      const availableProcesses = readyProcesses.filter(p => parallelGroup.includes(p.category));
...
export const generateProcessSchedule = (estimateData, options = {}) => {
  const { projectStartDate = new Date(), includeWeekends = false, bufferDays = 0 } = options;
...
const findCriticalPath = processes => {
  ...
  // 最長経路を返す
  const longestPath = paths.reduce((max, path) => (path.duration > max.duration ? path : max), {
    path: [],
    duration: 0,
  });
