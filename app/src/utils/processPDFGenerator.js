...
export class ProcessPDFGenerator {
  ...
  drawGanttChart(processes, x, y) {
    ...
    processes.forEach((process, index) => {
      ...
      // 工程バー
      this.drawProcessBar(x + nameColumnWidth, rowY - 4, chartWidth, process, startDate, totalDays);
  ...
  drawDateAxis(x, y, width, startDate, endDate) {
    ...
    for (let i = 0; i <= totalDays; i += 7) {
      ...
      if (i % 14 === 0) {
        ...
        this.doc.text(`${currentDate.getMonth() + 1}/${currentDate.getDate()}`, axisX - 5, y - 2);
      }
    }
  }
  ...
  drawProcessBar(x, y, maxWidth, process, startDate, totalDays) {
    ...
    // 工程期間テキスト
    this.doc.setFontSize(6);
    this.doc.text(`${processDays}日`, barX + barWidth / 2 - 4, y + barHeight / 2 + 1);
  }
    ...
    for (let day = 1; day <= daysInMonth; day++) {
      ...
      if (dayProcesses.length > 0) {
        ...
        dayProcesses.slice(0, 2).forEach((process, index) => {
          this.doc.text(this.truncateText(process.name, 8), cellX + 2, cellY + 12 + index * 4);
        });
...
export const printProcessSchedule = (processData, options = {}) => {
  const { format = 'gantt', paperSize = 'A4', orientation = 'landscape' } = options;
...
export const prepareProcessPDFForEmail = async (processData, options = {}) => {
  const { format = 'gantt', filename = 'process-schedule.pdf' } = options;
