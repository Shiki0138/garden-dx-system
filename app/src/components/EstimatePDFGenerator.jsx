...
export const generateEstimatePDF = async (estimateData, companyInfo = {}, options = {}) => {
  try {
    ...
    doc.setProperties({
      ...
      producer: 'Garden Estimate Generator'
    });
...
const addEstimateTotalSection = (doc, estimateData, margins, startY, pageWidth) => {
  ...
  doc.text(formatLandscapingCurrency(estimateData.subtotal || 0), rightAlign + boxWidth - 2, y + 6, { align: 'right' });
  
  y += 6;
  
  // 消費税
  const taxRate = estimateData.tax_rate || 0.10;
  const taxRatePercent = Math.round(taxRate * 100);
  doc.text(`消費税(${taxRatePercent}%)`, rightAlign + 2, y + 6);
  doc.text(formatLandscapingCurrency(estimateData.tax_amount || 0), rightAlign + boxWidth - 2, y + 6, { align: 'right' });
  ...
  doc.text(formatLandscapingCurrency(estimateData.total_amount || 0), rightAlign + boxWidth - 2, y + 6, { align: 'right' });
 ...
 * 見積書PDFダウンロード
 */
export const downloadEstimatePDF = async (estimateData, companyInfo, filename = null, options = {}) => {
  try {
    const pdf = await generateEstimatePDF(estimateData, companyInfo, options);
    
    const sanitizedEstimateNumber = (estimateData.estimate_number || 'No未設定').replace(/[^\w\s-_.]/g, '').substring(0, 50);
    const sanitizedCustomerName = (estimateData.customer_name || '顧客名未設定').replace(/[^\w\s-_.]/g, '').substring(0, 50);
