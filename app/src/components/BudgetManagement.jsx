...
const BudgetManagement = ({ projectId, estimateId, onBudgetUpdate }) => {
  ...
  const updatePurchaseAmount = useCallback(
    async (itemId, purchaseAmount, supplierId) => {
      try {
        ...
        if (response.ok) {
          setBudgetItems(prev =>
            prev.map(item => (item.id === itemId ? { ...item, purchaseAmount, supplierId } : item))
          );
          onBudgetUpdate?.();
        }
      } catch (error) {
  ...
  // 掛け率更新
  const updateMarkupRate = useCallback(
    async (itemId, markupRate) => {
      try {
        ...
        if (response.ok) {
          setBudgetItems(prev =>
            prev.map(item => (item.id === itemId ? { ...item, markupRate } : item))
          );
          onBudgetUpdate?.();
        }
      } catch (error) {
  ...
  const updateActualAmount = useCallback(
    async (itemId, actualAmount) => {
      try {
        ...
        if (response.ok) {
          setBudgetItems(prev =>
            prev.map(item => (item.id === itemId ? { ...item, actualAmount } : item))
          );
          onBudgetUpdate?.();
        }
      } catch (error) {
...
const BudgetItemRow = ({
  ...
  return (
    <TableRow>
      ...
      <TableCell>
        {isEditing ? (
          <Input
            ...
            onChange={e =>
              setEditValues({ ...editValues, purchaseAmount: parseFloat(e.target.value) || 0 })
            }
          />
        ) : (
      ...
      <TableCell>
        {isEditing ? (
          <Input
            ...
            onChange={e =>
              setEditValues({ ...editValues, markupRate: parseFloat(e.target.value) || 1.0 })
            }
          />
        ) : (
      ...
      <TableCell>
        {isEditing ? (
          <Input
            ...
            onChange={e =>
              setEditValues({ ...editValues, actualAmount: parseFloat(e.target.value) || 0 })
            }
          />
        ) : (
      ...
      <TableCell>
        <DifferenceAmount positive={difference >= 0}>{formatCurrency(difference)}</DifferenceAmount>
      </TableCell>
      <TableCell>
        {isEditing ? (
          <Select
            ...
            <form onSubmit={handleSubmit}>
              <FormGroup>
                ...
                <Input
                  ...
                  onChange={e =>
                    setFormData({ ...formData, purchaseAmount: parseFloat(e.target.value) || 0 })
                  }
                  placeholder="仕入額を入力"
                />
              </FormGroup>
              ...
              <FormGroup>
                ...
                <Input
                  ...
                  onChange={e =>
                    setFormData({ ...formData, markupRate: parseFloat(e.target.value) || 1.0 })
                  }
                  placeholder="掛け率を入力"
                />
                <FormHelp>販売価格: {formatCurrency(calculatedSellingPrice)}</FormHelp>
              </FormGroup>
              ...
              <FormGroup>
                ...
                <Input
                  ...
                  onChange={e =>
                    setFormData({ ...formData, actualAmount: parseFloat(e.target.value) || 0 })
                  }
                  placeholder="実績額を入力"
                />
              </FormGroup>
