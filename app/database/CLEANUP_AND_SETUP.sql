-- =================================================================
-- 庭想システム - クリーンアップ後の完全セットアップ
-- =================================================================
-- このスクリプトをSupabase SQL Editorで実行してください
-- ユーザーID: b591c784-71d9-4119-8123-c7fb47c6ed43

-- Step 1: 既存のRLSポリシーを削除
DROP POLICY IF EXISTS "Users can access their own company" ON companies;
DROP POLICY IF EXISTS "Users can access their own profile and company members" ON user_profiles;
DROP POLICY IF EXISTS "Users can access their company's customers" ON customers;
DROP POLICY IF EXISTS "Users can access their company's price master" ON price_master;
DROP POLICY IF EXISTS "Users can access their company's projects" ON projects;
DROP POLICY IF EXISTS "Users can access their company's estimates" ON estimates;
DROP POLICY IF EXISTS "Users can access their company's estimate items" ON estimate_items;

-- Step 2: 既存のテーブルがある場合はDROP（必要に応じて）
-- 注意: データが消えるので、必要であればバックアップを取ってください
...
-- Step 3: 必要なExtensionを有効化
CREATE EXTENSION IF NOT EXISTS "uuid-ossp";
CREATE EXTENSION IF NOT EXISTS "pgcrypto";

-- Step 4: 会社テーブル（Supabase用にシンプル化）
...
-- Step 5: ユーザープロファイルテーブル（Supabase Auth連携）
...
-- Step 11: RLS（Row Level Security）の設定
...
-- 単価マスタへのアクセス制御
CREATE POLICY "price_master_access_policy" ON price_master
    FOR ALL USING (
        company_id IN (
            SELECT company_id FROM user_profiles 
            WHERE id = auth.uid()
...
-- 見積へのアクセス制御
CREATE POLICY "estimate_access_policy" ON estimates
    FOR ALL USING (
        company_id IN (
            SELECT company_id FROM user_profiles 
            WHERE id = auth.uid()
...
-- 見積明細へのアクセス制御
CREATE POLICY "estimate_item_access_policy" ON estimate_items
    FOR ALL USING (
        estimate_id IN (
...
-- Step 16: 設定結果を確認
SELECT 'クリーンアップ＆セットアップ完了確認' as status;
...
SELECT 'Price Master:' as info, COUNT(*) as count FROM price_master;
...
-- URL: https://garden-dx-system.vercel.app (本番)
