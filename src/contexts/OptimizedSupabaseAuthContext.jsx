...
export const useOptimizedSupabaseAuth = () => {
  ...
  if (!context) {
    throw new Error('useOptimizedSupabaseAuth must be used within an OptimizedSupabaseAuthProvider');
  }
  return context;
};
...
export const OptimizedSupabaseAuthProvider = ({ children }) => {
  ...
  // セッション検証の最適化
  const validateSessionFast = useCallback((session) => {
  ...
  const value = useMemo(() => ({
    ...
    user, session, loading, error, isConnected,
    signInWithPassword, signOut, refreshTokenFast,
    hasRole, isAuthenticated, isManager, validateSessionFast,
    getPerformanceStats
  ]);
