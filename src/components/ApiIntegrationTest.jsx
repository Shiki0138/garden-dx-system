...
const ApiIntegrationTest = () => {
  ...
  // Mutation Hooks テスト
  const createEstimateMutation = useApiMutation(
  ...
  const updateEstimateMutation = useApiMutation(
    ({ id, data }) => estimateApi.update(id, data),
    {
  ...
  const deleteEstimateMutation = useApiMutation(
    (id) => estimateApi.delete(id),
    {
          ...
          <div className="estimates-test">
            ...
            <div className="test-section">
              ...
              <div className="test-status">
                状態: {estimatesQuery.isLoading ? '読み込み中...' : 
                      estimatesQuery.isError ? 'エラー' : 
                      estimatesQuery.isSuccess ? '成功' : '待機中'}
              </div>
            ...
            <div className="test-section">
              ...
              />
              <div className="test-status">
                状態: {estimateQuery.isLoading ? '読み込み中...' : 
                      estimateQuery.isError ? 'エラー' : 
                      estimateQuery.isSuccess ? '成功' : '待機中'}
              </div>
              {estimateQuery.data && (
                <div className="data-preview">
                  見積名: {estimateQuery.data.estimate_name}
                </div>
              )}
            </div>
            ...
            <div className="test-section">
              ...
              <div className="button-group">
                <button 
                  ...
                  disabled={createEstimateMutation.isLoading}
                >
                  {createEstimateMutation.isLoading ? '作成中...' : '見積作成テスト'}
                </button>
              </div>
              {testResults.createEstimate && (
                <div className={`test-result ${testResults.createEstimate.success ? 'success' : 'error'}`}>
                  {testResults.createEstimate.success ? '✅ 作成成功' : `❌ 作成失敗: ${testResults.createEstimate.error}`}
                </div>
              )}
            </div>
          </div>
        ...
        {selectedTab === 'pricemaster' && (
          <div className="pricemaster-test">
            ...
            <div className="test-section">
              <h4>単価マスタ取得</h4>
              <div className="test-status">
                状態: {priceMasterQuery.isLoading ? '読み込み中...' : 
                      priceMasterQuery.isError ? 'エラー' : 
                      priceMasterQuery.isSuccess ? '成功' : '待機中'}
              </div>
      ...
      {Object.keys(testResults).length > 0 && (
        <div className="test-results">
          ...
          {Object.entries(testResults).map(([testName, result]) => (
            <div key={testName} className={`test-result-item ${result.success ? 'success' : 'error'}`}>
