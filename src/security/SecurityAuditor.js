/**
 * Garden DX セキュリティ監査システム
 * OWASP Top 10準拠・RBAC・JWT・認証フローの包括的セキュリティチェック
 */

import { checkPermissionFast, getRBACStats } from '../utils/rbacOptimizer';
import { parseJWTOptimized, validateSessionFast, getSecurityMetrics } from '../utils/securityOptimizer';
...
export class SecurityAuditor {
    ...
    try {
      ...
      for (const test of escalationTests) {
        ...
        if (result !== test.shouldAllow) {
          issues.push(`権限エスカレーション脆弱性: ${test.user.role}が${test.resource}:${test.action}にアクセス可能`);
          score -= 15;
        }
      }
  ...
  async auditJWTSecurity() {
    ...
    try {
      ...
      if (typeof window !== 'undefined' && window.localStorage) {
        recommendations.push('JWT トークンの localStorage 保存は避け、httpOnly Cookie を検討してください');
      }

      // JWTクレーム検証
      const requiredClaims = ['sub', 'iat', 'exp'];
      recommendations.push(`JWTに必要なクレーム（${requiredClaims.join(', ')}）の存在を確認してください`);
  ...
  generateAuditReport() {
    ...
    return {
      metadata: {
        timestamp,
        auditor: 'Garden DX Security Auditor v1.0',
