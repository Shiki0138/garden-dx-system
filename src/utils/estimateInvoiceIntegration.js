...
export const formatEstimateForInvoice = estimateData => {
  return {
    ...
    tax_rate: parseFloat(estimateData.tax_rate) || 0.1,
...
export const convertEstimateToInvoice = async (estimateId, additionalData = {}) => {
  try {
    ...
    const invoiceData = {
      ...
      // 造園業界標準の税率設定
      tax_rate: estimateData.tax_rate || 0.1,
...
export const generateIntegratedInvoicePDF = async (invoiceData, estimateData = null) => {
  try {
    ...
    // 造園業界標準PDFを生成
    return await downloadLandscapingInvoicePDF(enhancedInvoiceData, companyInfo, estimateData);
  } catch (error) {
