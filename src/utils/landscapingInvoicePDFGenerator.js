...
const LANDSCAPING_STANDARDS = Object.freeze({
  ...
  landscaping: Object.freeze({
    ...
    standardUnits: [
      '本', 'm2', 'm3', 'm', '式', '台', '個', '回', 'kg', 't'
    ],
    workTypes: [
      '庭園造成工事', '植栽工事', '外構工事', 'エクステリア工事',
      '造園土木工事', '維持管理業務', '設計監理業務'
    ],
    warranties: {
      planting: '植栽活着保証1年間',
...
export const generateLandscapingInvoicePDF = async (
  ...
  doc.setProperties({
    ...
    creator: 'Garden DX System v1.0',
    producer: 'Garden Invoice Generator'
  });
...
const addLandscapingHeader = (doc, invoiceData, companyInfo, margins, startY, pageWidth) => {
  ...
  doc.rect(margins.left, y - 5, pageWidth - margins.left - margins.right, layout.headerHeight + 10, 'F');
  ...
  if (companyInfo.logo) {
    ...
    doc.setFontSize(8);
    doc.text('LOGO', margins.left + layout.logoWidth/2, y + layout.logoHeight/2, { align: 'center' });
  }
  ...
  if (companyInfo.business_license) {
    ...
    doc.text(`建設業許可：${companyInfo.business_license}`, margins.left + layout.logoWidth + 8, y + 20);
  }
...
const addCustomerInfo = (doc, invoiceData, margins, startY, pageWidth) => {
  ...
  customerInfo.forEach((info, index) => {
    if (info.trim() && index < 6) { // 最大6行まで
      const lineY = y + (index * 3.5);
      if (lineY < y + boxHeight - 15) { // 枠内に収まる場合のみ
        doc.setFontSize(fonts.normal.size);
        doc.setTextColor(colors.text);
...
const addLandscapingItemsTable = (doc, items, margins, startY, pageWidth) => {
  ...
  items.forEach((item, rowIndex) => {
    ...
    rowData.forEach((data, colIndex) => {
      ...
      // テキスト配置（造園業界標準アライメント）
      const cellPadding = 2;
      if (colIndex >= 3 && colIndex !== 4) { // 数量・単価・金額は右寄せ
  ...
  // 特記事項（造園業界慣習）
  const hasPlantingWork = items.some(item => 
    item.category && item.category.includes('植栽')
  );
  if (hasPlantingWork) {
...
const addLandscapingTotalSection = (doc, invoiceData, margins, startY, pageWidth) => {
  ...
  doc.text(formatLandscapingCurrency(invoiceData.subtotal || 0), rightAlign + boxWidth - 2, y + 6, { align: 'right' });
  
  y += 6;
  
  // 消費税（造園業界標準表記）
  const taxRate = invoiceData.tax_rate || 0.10;
  const taxRatePercent = Math.round(taxRate * 100);
  doc.text(`消費税(${taxRatePercent}%)`, rightAlign + 2, y + 6);
  doc.text(formatLandscapingCurrency(invoiceData.tax_amount || 0), rightAlign + boxWidth - 2, y + 6, { align: 'right' });
  ...
  doc.text(formatLandscapingCurrency(invoiceData.total_amount || 0), rightAlign + boxWidth - 2, y + 6, { align: 'right' });
...
const addLandscapingFooter = (doc, companyInfo, pageHeight, pageWidth, margins) => {
  ...
  if (companyInfo.company_seal) {
    doc.addImage(companyInfo.company_seal, 'PNG', sealX + 1, sealY + 1, layout.sealSize - 2, layout.sealSize - 2);
  }
};
